...
if [[ "$ENVIRONMENT" == "production" ]]; then
    ...
    if [ -f "$ENV_FILE" ]; then
        ...
        REQUIRED_VARS=("DB_USER" "DB_PASSWORD" "CLOUD_SQL_DATABASE_NAME" "SECRET_KEY" "SMTP_USERNAME" "SMTP_PASSWORD")
    ...
    # Set database name from environment
    ...
    # Generate the DATABASE_URL for Cloud SQL using variables
    ...
    ENV_VARS="$ENV_VARS,CORS_ORIGINS=https://wordbattle.binge-dev.de"
    ENV_VARS="$ENV_VARS,FRONTEND_URL=https://wordbattle.binge-dev.de"
    ENV_VARS="$ENV_VARS,RATE_LIMIT=60"
...
# Build gcloud run deploy command with WebSocket support
DEPLOY_ARGS=(
...
# Add Cloud SQL connection for both environments
if [[ "$ENVIRONMENT" == "production" ]]; then
    DEPLOY_ARGS+=("--add-cloudsql-instances=${PROJECT_ID}:${CLOUD_REGION}:${CLOUD_SQL_INSTANCE_NAME}")
else
