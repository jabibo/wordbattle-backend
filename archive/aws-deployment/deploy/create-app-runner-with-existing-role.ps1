# Create App Runner Service (assumes IAM role already exists)
param(
...
Write-Host "${Blue}🚀 Creating App Runner Service${NC}" -ForegroundColor Blue
...
# Construct IAM role ARN (skip verification due to permission limitations)
Write-Host "${Yellow}🔍 Using IAM role: $RoleName${NC}" -ForegroundColor Yellow
$RoleArn = "arn:aws:iam::${AccountId}:role/${RoleName}"
Write-Host "${Green}✅ IAM role ARN: $RoleArn${NC}" -ForegroundColor Green
...
try {
    Write-Host "${Yellow}📝 Service configuration:${NC}" -ForegroundColor Yellow
...
while ($ElapsedTime -lt $MaxWaitTime) {
    try {
        ...
        if ($Status -eq "RUNNING") {
            ...
            Write-Host "${Yellow}💡 Check AWS Console for detailed error logs${NC}" -ForegroundColor Yellow
            exit 1
        }
...
try {
    ...
    if ($Status -eq "RUNNING" -and -not [string]::IsNullOrWhiteSpace($ServiceUrl)) {
        ...
        Write-Host "${Yellow}   Current status: $Status${NC}" -ForegroundColor Yellow
    }
} catch {
