...
@router.post("/database/fix-games-schema")
async def fix_games_schema(
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        check_columns_query = text("""
            ...
            FROM information_schema.columns 
            WHERE table_name = 'games' 
            AND column_name IN ('name', 'ended_at');
        """)
        existing_columns = [row[0] for row in db.execute(check_columns_query).fetchall()]
        ...
        if 'name' not in existing_columns:
            db.execute(text("ALTER TABLE games ADD COLUMN name VARCHAR;"))
            results["actions_taken"].append("Added 'name' column to games table")
        
        # Add ended_at column if missing
        if 'ended_at' not in existing_columns:
            db.execute(text("ALTER TABLE games ADD COLUMN ended_at TIMESTAMPTZ;"))
            results["actions_taken"].append("Added 'ended_at' column to games table")
        
        # Create alembic_version table if it doesn't exist
        try:
            db.execute(text("SELECT COUNT(*) FROM alembic_version;"))
        except:
            ...
            db.execute(text("INSERT INTO alembic_version (version_num) VALUES ('0010_add_game_name_and_ended_at');"))
            results["actions_taken"].append("Created alembic_version table and set version to '0010_add_game_name_and_ended_at'")
        ...
        verify_query = text("""
            ...
            AND column_name IN ('name', 'ended_at');
        """)
        final_columns = [row[0] for row in db.execute(verify_query).fetchall()]
        
        return {
            "success": True,
            "message": "Games table schema fixed successfully",
            "existing_columns_before": existing_columns,
            "columns_after_fix": final_columns,
            "actions_taken": results["actions_taken"],
            "schema_complete": len(final_columns) == 2
        }
...
@router.post("/database/fix-games-schema-public")
async def fix_games_schema_public(
    db: Session = Depends(get_db)
):
    """Temporary public endpoint to fix missing columns in games table"""
    try:
        from sqlalchemy import text
