...
@router.get("/database/check-feedback-permissions")
async def check_feedback_permissions(
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Check feedback table permissions and access"""
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    try:
        ...
        feedback_exists = 'feedback' in tables
        ...
        if feedback_exists:
            columns = [{"name": col["name"], "type": str(col["type"])} for col in inspector.get_columns('feedback')]
            indexes = [{"name": idx["name"], "columns": idx["column_names"]} for idx in inspector.get_indexes('feedback')]
            foreign_keys = inspector.get_foreign_keys('feedback')
        ...
        # Test SELECT
        try:
            count = db.execute(text("SELECT COUNT(*) FROM feedback")).scalar()
            operations["SELECT"] = {"success": True, "count": count}
        except Exception as e:
            operations["SELECT"] = {"success": False, "error": str(e)}
        
        # Test INSERT (with rollback)
        try:
            db.execute(text("""
                INSERT INTO feedback (id, user_id, category, description, contact_email, status, created_at, updated_at) 
                VALUES ('test-permission-check', $1, 'general', 'Permission test', 'test@example.com', 'new', NOW(), NOW())
            """), [current_user.id])
            db.rollback()
            operations["INSERT"] = {"success": True, "message": "Test insert successful (rolled back)"}
        except Exception as e:
            operations["INSERT"] = {"success": False, "error": str(e)}
            db.rollback()
        
        # Test ORM access
        try:
            orm_count = db.query(Feedback).count()
            operations["ORM"] = {"success": True, "count": orm_count}
        except Exception as e:
            operations["ORM"] = {"success": False, "error": str(e)}
        
        return {
            "table_exists": feedback_exists,
            "all_tables": tables,
            "table_structure": {
                "columns": columns,
                "indexes": indexes,
