#!/usr/bin/env python3
"""
WordBattle Data Migration Script
================================

Automated, reusable solution to migrate all data from test environment to production.
...
Usage:
    python migrate-data-test-to-prod.py [--dry-run] [--verbose] [--force]
...
class DatabaseMigrator:
    """Handles secure data migration between test and production databases."""
    ...
    def connect_databases(self):
        """Establish connections to both test and production databases."""
        ...
        try:
            ...
            self.logger.debug(f"Test DB: {test_config['database']} on {test_config['host']}")
            
            # Load production database config  
            prod_config = self._load_environment_config('deploy.production.env')
            self.logger.debug(f"Prod DB: {prod_config['database']} on {prod_config['host']}")
    ...
    def validate_connections(self):
        ...
        try:
            ...
            with self.test_conn.cursor() as cursor:
                ...
                self.logger.debug(f"Test DB version: {test_version}")
            
            # Test production database
            with self.prod_conn.cursor() as cursor:
                cursor.execute("SELECT version();")
                prod_version = cursor.fetchone()[0]
                self.logger.debug(f"Prod DB version: {prod_version}")
    ...
    def migrate_table(self, table_name: str) -> bool:
        """Migrate a single table from test to production."""
        self.logger.info(f"📊 Migrating table: {table_name}")
        
        try:
            # Check if table exists in both databases
            if not self.table_exists(self.test_conn, table_name):
                self.logger.warning(f"⚠️  Table {table_name} does not exist in test database, skipping")
                return True
            ...
            self.logger.debug(f"Test DB {table_name}: {test_info['row_count']} rows")
            self.logger.debug(f"Prod DB {table_name}: {prod_info['row_count']} rows")
            
            # If no data in test table, skip
            if test_info['row_count'] == 0:
                self.logger.info(f"ℹ️  Table {table_name} is empty in test database, skipping")
                return True
            
            # Get all data from test table
            with self.test_conn.cursor(cursor_factory=RealDictCursor) as cursor:
            ...
            # Clear production table (careful!)
            with self.prod_conn.cursor() as cursor:
                cursor.execute(f"TRUNCATE TABLE {table_name} RESTART IDENTITY CASCADE;")
                self.logger.debug(f"Cleared production table {table_name}")
    ...
    def validate_migration(self) -> bool:
        ...
        for table_name in self.migration_tables:
            ...
            try:
                ...
                if test_info['row_count'] != prod_info['row_count']:
                    self.logger.error(
                        f"❌ Row count mismatch in {table_name}: "
                        f"test={test_info['row_count']}, prod={prod_info['row_count']}"
                    )
                    validation_passed = False
                else:
                    self.logger.debug(f"✅ {table_name}: {prod_info['row_count']} rows migrated correctly")
    ...
    def run_migration(self, specific_tables: Optional[List[str]] = None):
        ...
        self.logger.info("🚀 Starting WordBattle data migration")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate WordBattle data from test to production environment",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python migrate-data-test-to-prod.py --dry-run --verbose
  python migrate-data-test-to-prod.py --force
  python migrate-data-test-to-prod.py --tables users,games --verbose
        """
    )
    ...
    if not args.force and not args.dry_run:
        print("⚠️  WARNING: This will overwrite ALL data in the production database!")
